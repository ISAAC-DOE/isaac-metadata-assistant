...
@router.get(
    ...
    description=(
        ...
        "**AN ENTRY MAY CARRY `unavailable: true`, AND THEN `id` IS `null` AND THERE IS "
        "NO ANSWER KEY.** One entry is reported per stored blocking question, including "
        "one this application could not present as an answerable question — a stored "
        "value that is not a question at all, or one that names no kind. It is served "
        "rather than dropped, because dropping it would shorten this list below the "
        "record's own `pending_count` and make a still-blocked record look finished. "
        "`unavailable_reason` says what was found, `id`/`kind`/`blocker_key` are "
        "`null` (no answer key is invented — a fabricated one is refused by the answers "
        "operation with `422 unrecognized_field`), and `question` is preserved when the "
        "stored entry carried readable prose. Branch on `unavailable`, not on a pattern "
        "of nulls.\n\n"
