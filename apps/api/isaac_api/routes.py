...
# --- 23. TEMPORARY identity-observation probe (Workstream B, 2026-08-01) -------
#
# A measurement instrument, not a feature, and INTENDED TO BE REMOVED. Read
# `docs/identity-probe.md` (contract + removal plan) and the module docstring in
# `isaac_api/identity_probe.py` (why POST, why `consistent_with_previous_request`
# is omitted) before changing anything here.
#
# `docs/identity-trust-contract.md` §1 establishes that this application reads
# ZERO identity headers, and that the Authentik forward-auth edge in front of
# `/krish` is configured in `ISAAC-DOE/isaac-k8` — a repository this working tree
# cannot see. Q1-Q4 of that document ("which headers actually arrive?", "does the
# ingress strip a forged copy?") cannot be answered by reading code. They can
# only be answered by observing one real request at the pod. Q15 asks whether a
# presence-only probe may be enabled to do exactly that.
#
# THE RISK, STATED NOT MINIMISED. This operation is an ingress-configuration
# oracle: the set of candidates it reports present is precisely the list of
# header names worth forging, for an attacker who can reach the pod's Service
# directly and thereby bypass Authentik entirely (§2). `ISAAC_UI_API_KEY` is
# unset in the hosted deployment (§1.4), so the app-level bearer key that would
# have caught that caller is not in play.
#
# IT IS ENV-GATED, DEFAULT **ON**, AS A KILL SWITCH RATHER THAN AN ENABLER —
# and that is a deliberate, recorded weakening of what
# `docs/identity-trust-contract.md` §8 "Safety requirements" originally asked
# for. §8 specified default OFF. Turning a default-OFF switch on means editing
# `isaac-k8`, which Dean owns, and the instruction authorising this slice
# explicitly required no new secret and no infrastructure change. A gate nobody
# is permitted to open is not a safer probe; it is a probe that can never
# observe anything, leaving the header contract unmeasured indefinitely.
#
# So the switch is kept and its polarity flipped: setting `ISAAC_IDENTITY_PROBE`
# to a falsy value disables the probe WITHOUT a code deploy, which is the
# property actually worth having. Unset — every environment today — it observes.
#
# What carries the safety burden instead: this operation cannot return a value
# (see below), ISAAC consumes none of these headers so a forged one currently
# achieves nothing, and REMOVAL IS THE PLAN, not a hope — see
# `docs/identity-probe.md`. Observe once, then delete the endpoint.
#
# It never returns a header VALUE, or anything derived from one. Every string in
# the response is a compile-time constant, mechanically enforced by
# `identity_probe.assert_only_constant_strings` over the built payload — a
# strictly stronger guarantee than a substring leak scan, which can only prove
# that text it already knows about is absent.

_identity_log = logging.getLogger("isaac_api.identity_probe")

#: The deployment KILL SWITCH. Default ON; see the section comment for why the
#: polarity is inverted relative to trust-contract §8. Only an explicitly falsy
#: value disables; an unrecognised value is treated as "on", so a typo fails
#: towards the documented default rather than silently muting the probe and
#: producing an empty observation that looks like "no headers arrive".
_IDENTITY_PROBE_ENV = "ISAAC_IDENTITY_PROBE"
_IDENTITY_PROBE_FALSY = frozenset({"0", "false", "no", "off"})


def _identity_probe_enabled(env: Mapping[str, str] | None = None) -> bool:
    """Read the kill switch. Environment only — never a request, never a header."""
    source = os.environ if env is None else env
    return (source.get(_IDENTITY_PROBE_ENV) or "").strip().lower() not in _IDENTITY_PROBE_FALSY


class IdentityProbeRequest(BaseModel):
    """The probe body. ONE optional field: a canary the client plants in the
    candidate headers, so the response can say whether it survived the edge.

    `extra="forbid"` so a caller cannot smuggle an extra field into the handler.
    The canary travels in the BODY rather than the query string because
    `Dockerfile:50` runs uvicorn with default access logging, which writes the
    request line — query string included — to the pod's stdout. A body is not
    written to the access log.

    The length bound is enforced in the handler rather than by `max_length` here
    on purpose: a Pydantic `max_length` violation produces FastAPI's 422, whose
    `detail[].input` echoes the offending value straight back. The handler's
    check returns a fixed message that echoes nothing.
    """
...
@router.post(
    ...
    tags=[TAG_META],
    # DELIBERATELY PRESENT IN THE PUBLISHED CONTRACT. An earlier revision hid it
    # with `include_in_schema=False`, to avoid churning the three pinned
    # contracts in `test_about_and_openapi.py` on the way in and again on the way
    # out, and to avoid publishing a forgery recipe next to the supported
    # operations. Both points are real. They lose to a third:
    #
    # A TEMPORARY ENDPOINT THAT DOES NOT APPEAR IN THE APP'S OWN CONTRACT IS THE
    # KIND THAT IS NEVER REMOVED. Removal depends on someone noticing it exists,
    # and `/docs` plus the Settings > API Docs screen are where an operator
    # looks. Hiding it optimises the cost of this PR against the risk of leaving
    # an unadvertised identity-adjacent oracle in production indefinitely, which
    # is the wrong trade and is what `CLAUDE.md`'s lifecycle rule — "do not leave
    # an undocumented diagnostic endpoint in production" — exists to prevent.
    #
    # Obscurity was never the security control here in any case: the control is
    # that this operation cannot emit a value. The pin churn is accepted, twice.
    summary="Observe Which Identity Headers Reach the Application (Temporary)",
    description=(
        "**A temporary diagnostic, intended to be removed once it has been read "
        "once.** "
        "It reports which of a fixed compile-time list of seven candidate "
        "identity headers arrive at the application, a coarse structural shape "
        "for each, and whether a canary the caller planted in those headers "
        "survived the path in front of the app.\n\n"
        "It NEVER returns a header value or anything derived from one — not "
        "truncated, not hashed, not fingerprinted, not length-reported. It never "
        "returns a username, email, uid, subject, display name, group name, or "
        "entitlement. It never echoes `Authorization`, `Cookie`, or the canary. "
        "It reads only the seven allowlisted names and never iterates over the "
        "request's headers, so a header outside the list is invisible to it and "
        "its name can never appear in the response. Every string in the response "
        "is a compile-time constant.\n\n"
        "The canary travels in the request body rather than the query string "
        "because the container's access log records the request line, including "
        "the query string, but not the body.\n\n"
        "It deliberately does NOT report whether an identity is consistent across "
        "requests: that would require retaining a per-value fingerprint between "
        "requests, which is a cross-request correlation surface.\n\n"
        "Read-only. It changes no record, no workspace, no file, and no state of "
        "any kind, and it opens no connection. A deployment that sets the "
        "`ISAAC_IDENTITY_PROBE` switch to a falsy value turns it off without a "
        "code deploy, and it then reports `disabled` and reads nothing."
    ),
    response_description=(
        "The presence-only observation, or a sanitized disabled or error report "
        "carrying the same frozen top-level keys."
    ),
    responses={
        **_R_UNAUTHORIZED,
        400: {
            "description": (
                "The supplied canary exceeds the character limit. Nothing was "
                "observed, and the canary is not echoed."
            )
        },
    },
)
def post_identity_probe(
    request: Request,
    req: IdentityProbeRequest = Body(default_factory=IdentityProbeRequest),
):
    # The feature switch is read FIRST, before any header is touched. (The body
    # has necessarily been parsed by FastAPI before the handler runs; parsing is
    # not disclosure, and the parsed canary is not read on this path.)
    app_commit = _build_commit()
    generated_at = _now_iso()
    if not _identity_probe_enabled():
        _identity_log.info("identity_probe outcome=disabled")
        return identity_probe.sanitized_envelope(
            identity_probe.STATUS_DISABLED,
            app_commit=app_commit,
            generated_at=generated_at,
        )

    canary = req.canary if isinstance(req.canary, str) else None
    if canary is not None and len(canary) > identity_probe.MAX_CANARY_LENGTH:
        # Fixed text. The offending value is NOT echoed and NOT logged.
        _identity_log.info("identity_probe outcome=canary_too_long")
        return JSONResponse(
            status_code=400,
            content={
                "error": "canary_too_long",
                "message": (
                    "The canary exceeds the "
                    f"{identity_probe.MAX_CANARY_LENGTH}-character limit."
                ),
            },
        )

    # Fail closed. Nothing below may serialize a request, a header, or the
    # canary: the sanitized envelope is built from constants and the two
    # deployment-identity fields, and only the exception CLASS NAME — a code
    # constant, never `str(exc)` from anything that touched a header — is logged.
    # `request` itself is never repr()'d, formatted, or logged.
    try:
        ...
        # Last line of defence, over the payload that is about to be serialized.
        # Runs on the success path only: the envelope it degrades INTO must not
        # be able to raise, or fail-closed would stop closing.
        identity_probe.assert_only_constant_strings(payload)
        json.dumps(payload, ensure_ascii=True)
    except (KeyboardInterrupt, SystemExit):
        raise
    except BaseException as exc:  # noqa: BLE001 - the probe must never leak
        _identity_log.info("identity_probe outcome=error type=%s", type(exc).__name__)
        return identity_probe.sanitized_envelope(
            identity_probe.STATUS_ERROR,
            app_commit=app_commit,
            generated_at=generated_at,
        )

    _identity_log.info("identity_probe outcome=ok")
    return payload
