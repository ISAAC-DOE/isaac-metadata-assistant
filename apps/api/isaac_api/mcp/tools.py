    ...
    return (
        ...
        Tool(
            ...
            description=(
                ...
                "to walk the set a page at a time. Send any of the three and the response "
                "gains a `pending_page` block reporting `total`, `returned`, `withheld`, "
                "`complete` and `record_total` — read `pending_page.record_total` for the "
                "WHOLE record's open count, so a page you asked for can never be mistaken "
                "for the record's state. An unknown `run_id` is REFUSED, not answered "
                "with an empty list."
            ),
            scope=Scope.READ,
