...
RULE_BARE_INTEGER_ORDINAL = (
    ...
    "WHAT THE SECOND TOKEN MEANS IS CONFIRMED (DEC-47, 2026-09-17): it is a "
    "sample/electrode instance number, stated by the domain owner. The "
    "corroboration this rule used to rest on — contiguous, non-overlapping "
    "groups agreeing with the beamtime notes' own `Sample N` sections — is "
    "still true and is no longer what the reading depends on. THE RULE ID IS "
    "DELIBERATELY STILL `.v1`: the reading is byte-identical, because this "
    "recognizer always read the token to the same value and only the caveat "
    "beside it has been retired. A version bump would say a filename now reads "
    "differently, which it does not. STILL NOT A GRAMMAR: the meaning is "
    "confirmed, the POSITIONS are not, which is why recognition remains by "
    "shape and alias and this ordinal is the only ordering rule here."
)
RULE_PH_VALUE = (
