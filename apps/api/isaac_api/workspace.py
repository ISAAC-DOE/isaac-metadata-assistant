...
class ExportUnit:
    ...
    def record_path(self) -> Path | None:
        ...
        false of the experiment-level twin and is left visible rather than edited
        away.** :meth:`Experiment.record_path` gated on TRUTHINESS, not on
        ``is_record_id``, so a persisted ``record_id`` of ``"../planted_secret"``
        produced ``<records_dir>/../planted_secret.json`` and ``GET
        /api/experiments/{id}/artifacts`` returned **200** with the contents of a file
        outside ``records_dir`` (``"../../../far_away_secret"`` reached outside the
        whole workspace). Both halves are now gated — the parse in
        :meth:`Experiment.from_state` and the path in :meth:`Experiment.record_path` —
        so the claim this docstring wanted to make is true of the record namespace as a
        whole rather than of one class in it. The lesson worth keeping is not the
        traversal: it is that a "the only place" claim about a value that exists on two
        classes has to be checked on both.
        """
...
class Experiment:
    ...
    def record_path(self) -> Path | None:
        """``<records_dir>/<record_id>.json``, or ``None`` if the id is not a record id.

        THE SECOND OF TWO INDEPENDENT GATES on the same value, and it is deliberately
        redundant with the first. :meth:`from_state` already refuses a non-record-id
        through :func:`_as_record_id`, so on the read path this can only ever see a
        26-char id — but ``record_id`` is also a plain mutable attribute, assigned
        directly by the export route (``exp.record_id = result.record["record_id"]``)
        and by ``ExportUnit.mark_exported``, and an ``Experiment`` can be constructed
        without going through ``from_state`` at all. A parse-time guard cannot cover
        those; a guard at the point the string BECOMES A PATH can.
        Two gates is the posture this codebase already takes for the same shape of
        value: ``mcp/tools._EXPERIMENT_ID`` bounds an id in the published schema AND
        ``mcp/client._render_path`` refuses it again before a request is built.

        ``is_record_id`` rather than truthiness, and the difference is the whole
        defect: ``if self.record_id`` accepted ``"../planted_secret"`` and rejected
        ``0``/``{}``/``[]``/``False`` — which is exactly backwards, because
        :meth:`exported` asks ``is not None`` and so called those five "exported"
        while this method handed the reader ``None`` to call ``.read_text()`` on.
        """
        return (
    ...
    def sidecar_path(self) -> Path | None:
        ...
        Gated with the SAME predicate as :meth:`record_path` rather than a weaker one,
        because ``routes.get_artifacts`` reads both and reports one ``artifact.state``
        for the pair: a sidecar path built from a value the record path refused would
        make the two halves disagree about the same id.
        """
        rid = self.record_id
        return (
            self.records_dir / f"{rid}.evidence.json" if is_record_id(rid) else None
        )
    ...
    def from_state(cls, state: dict, *, session_id: str | None = None) -> "Experiment":
        ...
        raise as a fail-closed gate — the reset preflight, conflict healing, the local
        state read, the signature/rev read, the durable enumeration's ``unreadable``
        count, and the revision comparison — and each is enumerated in
        :func:`_readable_experiment_state`.

        **NO REQUEST-ANSWERING READ CALLS THIS DIRECTLY ANY MORE**, and that is what
        closed the measured whole-list 500: the three paths that turn a stored document
        into a response (:func:`list_experiments_with_hydration`,
        :func:`_load_all_experiments`, :func:`load_experiment`) go through
        :func:`_hydrate_experiment`, which normalises first. If you add a fourth,
        use that one — a reader that calls this directly can 500 a whole workspace on
        one malformed document, which is exactly the defect that was measured.
        """
        exp = cls(
            ...
            # ``record_id``, and this line was the measured counter-example to that
            # docstring's claim that the run was "the only place this slice could have
            # turned document content into a filesystem path". Measured over HTTP on
            # ``1ad1f8f`` with the value written into the persisted state document:
            #
            #   ``{}`` / ``[]`` / ``0`` / ``""`` / ``False``
            #       -> ``GET /{id}/evidence`` **500**, ``GET /{id}/artifacts`` **500**
            #       (``exported()`` is ``record_id is not None``, so a FALSY non-``None``
            #       said "exported" while ``record_path()`` — guarded by the same
            #       truthiness — answered ``None``, and the route called
            #       ``.read_text()`` on it)
            #   ``"../planted_secret"``
            #       -> ``GET /{id}/artifacts`` **200**, body carrying a JSON file from
            #       OUTSIDE ``records_dir``
            #   ``"../../../far_away_secret"``
            #       -> **200**, a file outside the whole workspace
            #
            ...
            # the traversal needs a persisted document written outside this
            # application (an operator edit, an out-of-band ``INSERT``, a future
            # importer). It is defence in depth, not a live breach. The read also only
            # resolves for a workspace where ``records_dir`` exists, i.e. one where
...
def _readable_experiment_state(state: object, experiment_id: str | None) -> dict:
    ...
    sense :func:`_hydrate_runs` owns that decision for a run entry and
    :func:`_hydrate_notes` for a note — so a reader and a writer cannot disagree about
    what a document contains.

    THE DEFECT, MEASURED OVER HTTP ON ``1ad1f8f``. ``Experiment.from_state`` read
    ...
    =========================================  ==========================  ==========
    ``id`` absent                              **500** ``KeyError``        **500**
    ``title`` absent                           **500** ``KeyError``        **500**
    ``created_utc`` absent                     **500** ``KeyError``        **500**
    ``rev: "x"``                                **500** ``ValueError``      **500**
    ``draft: "nope"``                          **500** ``AttributeError``  **500**
    ``source: "nope"``                         200                         **500**
    ``created_utc: 5`` (with a 2nd record)     **500** ``TypeError``       200
    =========================================  ==========================  ==========

    Every OTHER record's own detail route answered 200 throughout, which is what made
    it present as a silent whole-workspace outage rather than as one broken record:
    ``list_experiments_with_hydration`` catches only ``FileNotFoundError``. The
    ``created_utc: 5`` row needs a SECOND record to reproduce, because the raise is in
    ``out.sort(key=lambda e: e.created_utc)`` comparing ``int`` with ``str`` — it is
    the experiment-level twin of the ``sorted_runs`` defect ``_as_str`` records.
    ...
    the same defect was measured for runs, and the obvious move was to do that again.
    It was rejected after counting the consumers that depend on the raise, because it
    would have silently weakened all six at once:

    * :func:`_persisted_row` — the reset PREFLIGHT. An unhydratable document answers
      ``_UNREADABLE_ROW``, which compares unequal to every real row so the destructive
      reset REFUSES. A total reader would have handed it a plausible row instead.
    ...
    * ``submission_store._previous_unit_drafts`` — a predecessor that will not parse
    ...
    So the strict reader keeps its contract and its raise, every one of those six keeps
    its posture untouched, and the READ path — which is where the outage was — goes
    through this normaliser first. One assembly (``from_state``), one policy (here).

    NOTHING IS REPAIRED AND NOTHING IS INVENTED. Each key falls back to what a
    document MISSING that key already got: ``_as_str`` -> ``""`` (never ``str()``
    coercion, which would manufacture a title out of ``{"a": 1}``), ``_as_int`` -> 0,
    a non-dict ``source``/``draft`` -> ``{}``, a non-list ``answer_log`` -> ``[]``.
    The stored document is NOT rewritten by reading it. A later SAVE of the degraded
    record does write the fallbacks — the same disclosed consequence
    ``Run.from_state`` records for its own guards.

    THE DEGRADED RECORD STAYS BLOCKED, MEASURED RATHER THAN ASSERTED. Over HTTP after
    this change: a ``draft`` that is not an object reads ``status: in_review``,
    ``pending_count: 0``, ``exported: false``, and ``POST /export`` answers
    ``200 {"ok": false}``; a document missing ``id``/``title``/``created_utc``, or
    carrying ``rev: "x"``, reads ``status: needs_attention``, ``pending_count: 3``,
    and its export refuses the same way. So no degradation reaches ``ready_to_export``
    or ``done``, and none of them produces an artifact. ``in_review`` is the honest
    label for the first case and not a clean bill of health — it is the status for "no
    open questions this reader can see AND the export dry run fails", which is exactly
    the state of an unreadable draft. It is deliberately NOT turned into a synthetic
    ``pending`` entry: a blocker invented at read time would be written into the stored
    document by the next save, which is the repair ``CLAUDE.md`` §5 forbids.

    ``experiment_id`` IS THE DOCUMENT'S ADDRESS, NOT A GUESS. When the stored ``id`` is
    unusable the fallback is the directory (or path segment) the state was READ FROM,
    which is the same rule two other readers already apply in the opposite direction:
    ``stored_experiments`` skips a row whose ``state["id"] != <row key>`` and
    ``_heal_from_conflict`` skips a winner whose ``state["id"] != self.id``. Both treat
    the address as authoritative over the document's claim about itself. It matters
    beyond cosmetics: ``Experiment.dir`` is ``scope_root / self.id``, so an ``id`` of
    ``""`` would alias the SCOPE ROOT and a later save would write
    ``<scope_root>/experiment.json`` — a real file, outside every experiment directory,
    that no enumeration would ever read again. A document whose id disagrees with its
    address is left disagreeing; only an UNUSABLE one adopts it.
    """
    if not isinstance(state, dict):
        # Not a document at all. Everything below would read ``.get`` off it.
        state = {}
    readable = dict(state)
...
def _hydrate_experiment(
    ...
    *,
    ...
    strict. Every path that answers a request with a stored document goes through
    here; the six fail-closed consumers listed there deliberately do not.
    """
...
def list_experiments_with_hydration(
    ...
    for d in _experiment_dirs(scope_root(session_id)):
        ...
        # detail route still answered 200 — see ``_readable_experiment_state`` for the
        # measured table. STILL UNCOVERED, and named rather than implied: the
        ...
        # a separate policy decision (a blank row at its address, or nothing) and is not
        # made here.
...
def load_experiment(experiment_id: str, session_id: str | None = None) -> Experiment | None:
    ...
    if not state_path.exists():
        ...
        try:
            ...
            if not state_path.exists():
                return None
    # DEGRADES RATHER THAN RAISING, for the same reason the enumerations do: the owner
    # of a malformed record did nothing wrong and must not be shown a 500 in place of
    # their record. ``experiment_id`` is passed as the address fallback — it is the
    # segment this path was built from, so a document whose own ``id`` is unusable is
    # still addressable and its ``dir`` cannot alias the scope root.
...
def _load_all_experiments(session_id: str | None = None) -> list[Experiment]:
    ...
    for d in _experiment_dirs(scope_root(session_id)):
        ...
        # FAIL-CLOSED GATE IS UNCHANGED BY THAT: ``_persisted_row`` re-reads each
        # document through the STRICT ``Experiment.from_state`` and still answers
        # ``_UNREADABLE_ROW`` for one that will not hydrate, so a plan digest over a
        # malformed document still refuses. This function only decides what the
