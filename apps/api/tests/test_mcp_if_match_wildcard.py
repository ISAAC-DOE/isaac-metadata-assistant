...
_WILDCARD_CASES = (
    ...
    # The NINTH write, added with MCP-005's `isaac_capture_transcript`. Here for the
    # seventh's and eighth's reason: this file's coverage assertion is DERIVED from
    # `OPERATIONS`, so a write added without a case turns
    # `test_every_mutating_operation_is_covered_by_this_file` red rather than shipping
    # with an unguarded wildcard and a green suite. It costs `PROPOSALS_WRITE`, which
    # changes nothing here — the wildcard refusal is `client._render_headers`', and it
    # is scope-blind.
    ("create_transcript", {"experiment_id": _ID}),
)
...
def test_every_tool_that_declares_if_match_says_the_wildcard_is_refused():
    ...
    assert set(descriptions) == {
        ...
        "isaac_capture_note",
        # `isaac_capture_transcript` (MCP-005) is the third, and the reason is the
        # same one again: both the notes and the proposals it stores live inside the
        # record's own state document, so one capture moves the record's `rev`.
