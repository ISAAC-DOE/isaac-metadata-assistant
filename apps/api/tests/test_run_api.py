...
_DISCLOSED_TRUTH_PATH_CHANGES: dict[str, str] = {
    ...
    "src/isaac_records/draft_validator.py": (
        ...
        "`validate_draft` walked the ten containers it knows about without asking what "
        ...
        "500. NOT ALL TEN RAISED, and this entry said they did until an independent "
        "review measured otherwise; the correction is recorded here rather than "
        "silently applied, because the false version concealed the one verdict this "
        "change moves. EIGHT raised unconditionally (`meta`, `fields`, `attribution`, "
        "`assets`, `descriptors_outputs`, `implicit`, `series`, `links`). `qc` raised "
        "only when a series was present, and the new guard sits inside that same `if "
        "series:`, so `qc` behaves identically before and after. `block_evidence` "
        "raised only when a series, a link, a contributor or the qc gate looked it up, "
        "so a draft with none of those VALIDATED CLEAN — and that is the single verdict "
        "flip: such a draft now reports PASS -> FAIL and `draft_ok` True -> False. It "
        "is a flip toward refusal on a document that could never have exported: "
        "`export.build_sidecar` does `(draft.get(\"block_evidence\") or {}).items()`, "
        "so it raised `AttributeError` out of `export_draft`, and that third crash is "
        "now a clean refusal too. NOTHING THAT PREVIOUSLY EXPORTED STOPS EXPORTING. "
        "WHY THE TRUTH CORE AND NOT THE ROUTE: `validate_draft` is also "
        ...
        "All 14 guards in this slice were mutation-tested. Delete once landed."
    ),
}
