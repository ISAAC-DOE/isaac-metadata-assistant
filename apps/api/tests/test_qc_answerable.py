...
def test_the_route_does_not_carry_its_own_copy_of_the_verdict_enum(app):
    ...
    for verdict in ("compromised",):
        assert f'"{verdict}"' not in source, (
            f"routes.py spells {verdict!r} itself instead of deferring to is_qc_shaped"
        )
...
def test_the_canonical_scenarios_still_carry_the_verdicts_they_shipped_with(app):
    ...
    seeded = tutorial_client(app)
    for exp in seeded.get("/api/experiments").json()["experiments"]:
