...
def test_edit_corrects_answered_field_bumps_once_and_reflects_new_value(client):
    ...
    persisted = ws.load_experiment(ws.SEED_READY_ID).draft
...
def test_edit_missing_if_match_returns_428(client):
    ...
    # no mutation
    assert ws.load_experiment(ws.SEED_READY_ID).draft["assets"]
...
def test_edit_unknown_field_writes_nothing(client):
    before = ws.load_experiment(ws.SEED_READY_ID).draft
    ...
    assert r.status_code == 422, r.text
    after = ws.load_experiment(ws.SEED_READY_ID).draft
...
def test_edit_malformed_sha256_is_a_no_op_never_written(client):
    ...
    raw = next(a for a in ws.load_experiment(ws.SEED_READY_ID).draft["assets"] if a["uri"] == RAW_URI)
