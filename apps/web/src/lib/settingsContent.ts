  ...
  return [
    ...
    },
    {
      ...
      detail: durable
        ...
        : unavailable
          ? `The workspace is stored as files on the server for this deployment, and is not shared between deployments. ${walkthroughStorage} A database is configured for this deployment to hold experiment records and the backend has recorded that they were not reaching it, which is what it means by reporting persistence as unavailable. Creating one may fail outright, and nothing this screen has read establishes where a record created now is stored, or that it is stored durably.`
          : ephemeral
    ...
    },
    {
      ...
      summary: durable
        ...
        : unavailable
          ? 'A database is configured for this deployment to hold experiment records and the backend has recorded that they were not reaching it, and nothing this screen has read establishes that a record created now is stored durably; the workspace is files on the server, discarded with the deployment’s temporary storage.'
          : ephemeral
      ...
      detail: durable
        ...
        : unavailable
          ? "The backend reports persistence as unavailable: a database is configured for this deployment to hold experiment records and the backend has recorded that they were not reaching it. Creating one may fail outright, and nothing this screen has read establishes that a record created now is stored durably. The workspace is not stored in a database. Workspace state is written as files in a working directory on the server, so restarting the backend process does not by itself clear it; it is not durable, is not shared between deployments, and is discarded whenever the temporary storage it sits on goes away — this screen cannot say when that will be. The isolated SLAC test database that the protected, read-only diagnostic may read is not the workspace's storage and is not the database this state is about: nothing from it is written here, no record is modified, and only sanitized aggregate results are returned."
          : ephemeral
      ...
      more: {
        ...
        text: 'Assistant conversations stay in the browser tab that created them, but they are not held only in memory: the transcript is written to sessionStorage in that tab, so it survives a page reload and is erased when the tab closes. It is never sent to a server, never logged, and never written to localStorage or IndexedDB. Only the most recent 40 messages are kept. Record verdicts are dropped by field, and a message whose TEXT contains a credential, an absolute file path or a long hex digest is not stored at all — the whole text is withheld rather than the offending fragment snipped out, because deciding where a file path ends cannot be done reliably and a redaction that guesses wrong would leak. That is broader than strictly necessary for a credential or a digest, whose boundaries are clean; one rule is applied to all three rather than three rules with one weak link. The archived copy says so in place of the text, and the answer you were given at the time is unaffected.',
      },
    },
    {
      ...
      detail: `Only one thing in this build has a stated maximum age, and it is the worked-example walkthrough: the server sets how long an unfinished one may sit and states that limit to the page when the walkthrough opens, though no screen here shows the number. What Is Stored describes what becomes of one that passes it. Nothing else is kept to a schedule. The workspace has no expiry of its own and lasts as long as the working directory it sits in, which is the directory What Resets is about. The verification report is held in the server process memory for a bounded period before a recomputation is offered, is shown with its own age, and is gone when that process restarts; the protected, read-only diagnostic keeps none of what it reads. ${
        durable
          ...
          : unavailable
            ? 'How long an experiment lasts depends on where this deployment stores experiments, and here a database is configured to hold them but the backend has recorded that records were not reaching it, so creating one may fail outright and nothing this screen has read establishes how long a record created now lasts.'
            : ephemeral
