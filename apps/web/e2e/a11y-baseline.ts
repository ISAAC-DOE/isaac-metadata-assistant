...
export const A11Y_BASELINE: readonly BaselineEntry[] = [
  {
    ...
    counts: {
       ...
       *   width-390          73 -> 74
       *
       * THE LINUX COLUMN IS NOT TOUCHED, and it is now KNOWN TO BE STALE. This is a
       * TEXT-LENGTH change, which is precisely the wrap-dependent case where this
       * file's own rule forbids arithmetic on the other platform: only the platform
       * you measured may be edited, and the other column comes from a CI run. So five
       * of these keys keep their last linux reading beside a new darwin one, and CI
       * will report five GREW messages naming its own figures — transcribe those.
       *
       * TWO OF THE SEVEN COULD NOT BE WRITTEN THAT WAY, and they are flagged rather
       * than smoothed. At laptop-1024x768 and tablet-768x1024 the NEW darwin number
       * happens to equal the OLD linux number (57 and 72), and the well-formedness
       * guard rejects a per-platform pair whose halves are equal. A scalar is the only
       * legal expression, and a scalar in this file means "identical on both
       * platforms" — so those two cells now ASSERT a linux value that no run has
       * produced since `542d757`. That is the type system's limitation, recorded here
       * exactly as the 2026-08-04 note records it, and not a claim about linux.
       */
      'settings-explorer@desktop-1280x800': { darwin: 57, linux: 55 },
      'settings-explorer@laptop-1024x768': 57,
      /*
      ...
      'settings-explorer@tablet-768x1024': 72,
      ...
      'settings-explorer@mobile-375x812': { darwin: 73, linux: 72 },
      ...
      'settings-explorer@zoom-200': { darwin: 70, linux: 68 },
      ...
      'settings-explorer@width-320': { darwin: 76, linux: 73 },
      ...
      /* /api/about DESCRIPTION, 2026-08-28: 52 -> 67, still a scalar — both faces
         ...
         `settings-explorer@desktop-1280x800`. */
      'settings-explorer@width-390': { darwin: 74, linux: 72 },
...
export const A11Y_BASELINE_TOTAL_NODES: Readonly<Record<BaselinePlatform, number>> = {
  ...
  //   2430 - 7 - 154 = 2269
  //
  // Both totals were recomputed by summing `platformCount` over every entry rather
  // than by adding these deltas by hand. If CI disagrees with either number, correct
  // THE NUMBER from the CI output; never loosen the assertion.
  linux: 2269,
};
