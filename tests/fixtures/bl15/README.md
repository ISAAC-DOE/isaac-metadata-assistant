...
| `semantics/multi_operator_corpus/` | **added 2026-09-22** for `apps/api/tests/test_historical_semantics.py`: a whole small archive (9 acquisitions, 2 scan exports each, a readme, a method macro, a `newfile` macro and a notes file) built to exercise conventions, operators, conflicts and signals together. Legacy 01–05 use the build's BL15-2 filename vocabulary and 06–08 a second, test-only vocabulary (`alk`, `fresh`); three invented operators ("Operator Alpha/Beta/Gamma (synthetic)") are named per sample section, one of them across both vocabularies; 01–03 declare `beforeCycling` in `#F` while their names say `after500Cycling` (a systematic rename); legacy 05 is carried by TWO acquisitions; the Vortex columns carry signal on `vortDT` only, `vortDT2` only, or both, so the HERFD selector meets every branch; unit 04's second scan states a different `#D` date (one field-level disagreement at a writable path); the notes say "SYNTHETIC: held at room temperature throughout" and nothing numeric; the `Notes` cells hold short invented remarks, each marked SYNTHETIC, one of them a multi-line cell in a row that is not last, and the last row of the last table is followed directly by a line of prose that is not part of the cell. **No sentence of the real archive is reproduced** (corrected 2026-09-22 after an independent review): every procedure cell begins with the notes reader's own recognition phrase (`_ECHEM` in `bl15/notes.py`, already public in shipped code) and is invented after it, and the table headings are the structure names the reader matches. Element `Zz` (not an element), years 2099/2100, energies 1000–1100. The notes file, readme and both macros state they are synthetic in their first line; each SPEC acquisition carries a `#C SYNTHETIC FIXTURE` header comment; the per-scan `.dat` exports carry no marker of their own (a real export begins with `#S`) and are covered by this row. It was generated by a script the slice kept outside the repository, so to change it, edit the files and this row together |
