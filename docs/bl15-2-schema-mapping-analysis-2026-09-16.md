...
| same-sample relationship | the shared sample/electrode token (`_02_`) across a legacy-number range | `links[].rel = same_sample_as`, `basis = same_sample_id` | ~~Deterministic **only** if the second token really is the sample instance — which is **domain question 1**. Until answered this is a candidate, not a mapping.~~ **CORRECTED 2026-09-17: `Q1` is ANSWERED.** Angel confirmed the second token is a sample/electrode instance number (`DEC-47`), so the premise this cell was waiting on holds. It is struck rather than deleted because "until answered" is the kind of clause a future session acts on. What is still open is narrower and is about the schema, not the token: an instance number could land at `sample.sample_id` or `sample.library.sample_no`, and `sample_form` is required alongside. |
...
| reaction | 15 members incl. `OER`, `water_splitting`, `surface_oxidation` | The notes name the reaction in their own words, in terms that fit three of these members; the corpus does not choose between them. *(Wording withheld: it is note prose, not a filename.)* |
| cell type | 10 members incl. `three_electrode`, `flow_cell` | The notes describe a three-electrode cell for the JK samples and a flow cell for TiO2 — so this is **per sample group**, not per beamtime. |
