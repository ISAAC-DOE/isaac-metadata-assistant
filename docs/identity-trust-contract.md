...
| Audit attribution | **No actor** — timestamps only | `answer_log` at `workspace.py:2506` — ~~`workspace.py:245`~~, corrected 2026-08-24: that line is an `ISAAC_RUNTIME_MODE` docstring, and this sentence kept a stale line number in the same breath as telling the reader not to trust one — appended at **three** sites (two until 2026-08-19, when the per-run write path was added) as `{"applied"/"edited": …, "at": ts}`, the run-level entry also carrying `run_id` — re-derive with `rg -n 'answer_log\.append' apps/api/isaac_api/routes.py` rather than trusting a line number, which is why the old ones are dropped here | n/a | ephemeral | — | changes are untraceable to a person |
