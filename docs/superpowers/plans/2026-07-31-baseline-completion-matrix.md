...
| Accessibility defects found (A1–A8 below) | **yes** — recorded | **partial** — **A1, A2 (both critical), A4, A5 and A6 FIXED**, baseline entries deleted so a regression fails as `new`; **A3, A7 and A8 OPEN and deliberately deferred**, each still baselined at an exact node count | orch | §3B. A3 is a PALETTE DECISION and is the owner's, not a slice's (`styles/tokens.css:3-5`); A7/A8 are the record-chrome reflow slice. The one remaining a11y finding is serious, the 2 open layout findings are unrated |
...
| **A6** | `landmark-unique` | moderate | **FIXED 2026-08-26** | Two unnamed `role="search"` landmarks on Endpoint Explorer. The `aria-label` added for A1 sat on the `<button>`, **not** on the `role="search"` wrapper `<div>`. Both landmarks now carry their own `aria-label` — "Site search" (`SearchDialog.tsx`) and "Endpoint search" (`settings/ApiDocs.tsx`). Its baseline entry — 14 nodes once the narrow sweep landed, not the 10 recorded here — is DELETED, **−14 nodes on both platforms** |
...
- **A4** changes keyboard-focus semantics of scroll containers (adding `tabindex="0"` introduces new
  ...
  copy, not markup. **All three were closed on 2026-08-26** — see their rows above. The reasoning
  here is kept rather than deleted, because each concern was real and each shows in the fix: the new
  tab stops are unconditional and documented at the call site, `expectH1: false` went in the same
  change as the heading, and the two landmark names were chosen for distinctness after a first
  attempt that reused one string for two elements had to be withdrawn.
...
**Baseline impact of leaving A3, A7 and A8 open** (A4–A6 are closed; the sentence used to read
"A3–A8" and is corrected rather than deleted so the change of status is visible): each remains enumerated in `e2e/a11y-baseline.ts` or
