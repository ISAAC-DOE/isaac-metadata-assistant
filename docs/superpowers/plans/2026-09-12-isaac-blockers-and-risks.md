...
**TWO SPLIT WATCH LISTS EXIST AS OF 2026-09-17, AND THEY ARE THE FASTEST ANSWER TO "AM I
BLOCKED?"** — [`docs/external-watch-lists-2026-09-17.md`](../../external-watch-lists-2026-09-17.md).
The matrix in §1 below mixes owners, which is right for a register and wrong for a session trying to
decide whether to proceed. The split lists carry one column this file does not: **what each answer
unlocks**, so a session can check mechanically whether a given row is its blocker. It almost never
is. They also name a **third** category — items that are Krish's own or whose owner is **unknown** —
which a two-list framing would otherwise disguise as awaiting an external reply.
...
| **EXT-08 — migration `0005`: approval, application, backfill, Stage-2b cutover** | Krish (approval) + operator (application) | authoritative run-projection cutover | **hosted `run_projection.last_pass.unavailable: 3`** — all three hosted experiments have an unreadable projection (observed 2026-09-12). `0003`/`0004` are **owner-approved 2026-08-17 and applied nowhere**; ~~`0005` is **not approved**~~ → **`0005` is owner-approved 2026-09-17 and applied nowhere — so ALL THREE are approved and none is applied** — and its backfill has **never run anywhere**. **The approval moved one blocker and not the others:** `EXT-08`'s remaining content is entirely the operator's three sequenced acts plus the backfill, and Krish owes nothing further on it | YES | **zero migrations are required by this plan** — this blocker gates no proposed slice |
...
|---|:-:|:-:|---|---|
| **Silent ambiguity collapse** — a regex picks one of two candidates | ~~5 (currently occurring)~~ → **1** | **5** | **FIXED `47fdbe30`** — a **two-pass** match: `finditer` **plus** a restatement read. ~~`search`→`finditer`~~ alone does **not** fix it (the rule is anchored on the word *temperature*, said once in that sentence). A **third defect of the same family** was found on the way: the non-kelvin guard asked *"any kelvin reading in this segment?"*, so `"425 K, maybe 430 C"` produced 425 **and no abstention** — `430 C` vanished silently too | **Negative control run both directions and recorded verbatim**: unmodified → `assert [425] == [425, 430]`, *"Right contains one more item: 430"*, 12 failed / 18 passed, exit 1; fixed → exit 0. Plus a **five-mutation matrix** proving each of the five guards individually load-bearing. **M1 is the lesson: `finditer`→`search` left every VALUE assertion green** — an inert-looking mutation that is actually load-bearing for a *different* property (it stops an instant labelled "ended" being proposed as a START) |
