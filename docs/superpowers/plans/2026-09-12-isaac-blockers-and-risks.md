...
**Status:** PROPOSED — planning artifact. **Created:** 2026-09-12 · **Base:** `main` @ `2f9a1133`
...
| **EXT-10 — a representative BL15-2 corpus + expert ground truth** | **Krish + Angel** | **all of Pillar 2's parsers and evaluation** | **measured: zero `.mac`, zero `.xlsx`/`.xls` in the entire tree**; `examples/` is generated by `scripts/make_synthetic_examples.py` with fictional people and a fictional year-2099 session; `openpyxl` is not a dependency | **PARTIALLY** — the Import Session shell and review surface proceed; **no parser does** | **issue `HIST-000`, the data request, immediately.** It costs nothing and is the gate |
...
| Authentication | OAuth **and** auth type **`none`**, documented **"Supported"** | — | **an authless remote MCP server is vendor-permitted, so OAuth is ISAAC's choice, not a vendor gate.** A stronger, cheaper argument to Dean — and free, since ISAAC already built OAuth. **It does not close EXT-01:** an OAuth bearer yields a `ServicePrincipal` | — |
