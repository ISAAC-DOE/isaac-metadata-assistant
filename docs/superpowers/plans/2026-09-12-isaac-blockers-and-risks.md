...
| **Removing provenance to simplify the UI** | **4** | **5** | `EVG-001` is a hard prerequisite of `EVG-002` — `/provenance` loses its only frontend caller otherwise | PR ordering enforced; `derived_from` chains visibly reachable without the graph |
...
| `CLAUDE.md` §16/§17 | "~41" files in `docs/superpowers/plans/` | **44** |
...
| `docs/browser-accessibility-testing.md` | A11Y-01 is "an open palette decision" | **closed** — `tokens.css:220` + `palette-contrast.test.ts`; zero hits for "A3", "4.54" or "#626c77" in that doc |
| `docs/` (several) | `https://isaac.slac.stanford.edu/krish/api/mcp` cited as live | returns **404** |
