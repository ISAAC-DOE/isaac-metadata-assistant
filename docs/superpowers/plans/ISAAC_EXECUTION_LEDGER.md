...
RE-DERIVED AT RUN OPEN, every line from a command:
  ...
  *** THE PREVIOUS HEADER WAS STALE IN A WAY WORTH NAMING, because it is the exact
  ...
  WHAT THIS RUN SHIPPED — the full account, with every figure's command, is
  docs/session-closure-2026-09-18.md. In brief:
    #271 c9ffed06 v0.0.254  the migration operator index + DEC-51
    #272 b7120462 v0.0.255  ACT-003b Activity UI polish, after ACT-003's FIRST
                            independent review (3 Important defects its own suite passed)
    #274 78e9371e v0.0.256  browser-QA evidence, negative-controlled
    #273 44760336 v0.0.257  ACT-004 Statistics-from-Activity
    #275 (open at write time) CTX-004 Extended Context made inspectable
  Each tag resolved with `git rev-list -n1 <tag>`, not read off a workflow log.
```
...
is a switch, a decision, or a person — not product code.
  ...
  | hosted QA of this image | n/a | Krish | `docs/krish-qa-packet-2026-09-15.md` — **6 numbered checks** (each saying why it needs a human), whose §6 "new in this session" table grew **4 → 6 rows** on 2026-09-15 with the Settings ADVANCED group and the Add-a-whole-import control. The import row deliberately asks for the three states IN ORDER — send with no run named (it must refuse the WHOLE thing), then with the run, then again (all `already there`) — because two of the three are refusals and a happy-path pass would miss both. The packet now also carries **one DECISION, not a check**: whether prose should be capped to a reading measure app-wide, which is the owner's call and is the only way to clear the `line-length` finding given the recorded "don't cap prose narrow" |
...
| `BL15R-002` | Content-led source classifier | **DONE 2026-09-16.** `bl15/classify.py`, `overridable=True` always and a `confidence` of `content`/`path`/`unknown`. **`.mca` is tested BEFORE the `#F` branch and the order is the point** — both `.mca` files begin with `#F`, so a content-first test calls detector products acquisitions; the code says why the obvious ordering is wrong for that one case and why it does not weaken content-led classification (`run29` and `alignment` need content because their names carry nothing; for `.mca` the extension is the MORE SPECIFIC fact) | `BL15R-001` |
| `BL15R-003` | `SSRL BL15-2 — Angel-style historical naming profile v1` + token reader | **DONE 2026-09-16.** `bl15/profiles.py` + `filenames.py`. A REGISTRY: the profile records the owner's convention verbatim and then shows with five measured counterexamples that it is a VOCABULARY, not a grammar — tokens are recognised by shape and alias, never by position. **44 normalisation rules, each warranted by something the corpus states**, and case-folding cites a DIFFERENT rule from an exact match so a reviewer can tell them apart; nine are declared UNEXERCISED (no filename carries a pH or a flow rate) and asserted as such. A test rejects any rule string not in the declared set, so a rule cannot be invented at a call site. **The architecture requirement is made checkable**: a test builds a second profile with different aliases and reads a stem through it without touching any reader. Measured over the corpus: only **4** `unknown_token` items over **3** distinct literals | `BL15R-001` |
| `BL15R-004` | SPEC acquisition reader | **DONE 2026-09-16, and the pairing is PROVEN BY AN INDEPENDENT ORACLE rather than by inspection.** The root acquisition keeps motor NAMES in one block and positions in another; the `.dat` export writes the same state as `name=value`. The two readers share no pairing code, and on one scan they agree on **172 of 172 motors, 0 mismatches**. A `#P` line whose field count does not match its `#O` line is REFUSED with both counts, because a mispaired mapping is worse than none. `#E` and `#D` are **not reconciled** — two statements, and a disagreement belongs to a scientist. `#C` is raw evidence and never an identity | `BL15R-001` |
| `BL15R-005` | `.dat` scan reader | **DONE 2026-09-16** — 908/908 parsed, 0 refused. Deliberately does **not** share the `#O` pairing code: a mispairing is impossible in this format, so carrying its refusal path would invite a future change to make one format's assumption true of the other | `BL15R-001` |
| `BL15R-006` | `.mac` reader | **DONE 2026-09-16** — 60/60 parsed, **156** `newfile` declarations over **95** distinct targets. Nothing is evaluated, and that is asserted by walking the module's **AST** for forbidden imports and call targets, with a negative control proving the walk fires. The acquisition method's four-argument signature is read from the archive's OWN printed help text, and a wrong argument count yields **nothing** rather than a guessed mapping | `BL15R-001` |
| `BL15R-007` | Safe folder/ZIP inventory | **DONE + VERIFIED AGAINST THE REAL ARCHIVE AND ITS REAL ZIP 2026-09-16.** `bl15/archive.py`. Every guard has its own test, with negative controls where one could pass vacuously (`..hidden` is legal; legitimately compressible text is accepted). **The load-bearing one: a member LYING about its size** — its test first asserts the declared-ratio pre-check *cannot* catch it (declared ratio under 1), then that the READ stops at the cap, because trusting the central directory is the whole vulnerability. Nothing is executed, asserted by an **AST sweep** rather than a comment. **Three-way agreement**: folder walk, ZIP walk and `scripts/bl15_corpus_report.py` all read 1,192 entries / 89,163,651 B / 96 duplicate groups over 193 files / 0 refusals, with 14 checks including byte-identical `to_state()` and digests matching an independent `os.walk`+`hashlib` pass. **Two traps recorded in the code:** the ZIP wraps everything in one directory (unstripped, the same corpus read two ways gives a different path for every file), and `zipfile.writestr` stores permission bits with NO file-type bits, so an `S_ISREG` check refuses every ordinary member of every `zipfile`-written archive — 18 tests failed on it and the real corpus HID it (`external_attr` is 0 for all 1,194 members) | `BL15R-001` |
| `BL15R-008` | `readme.txt` shared beamtime context | **DONE 2026-09-16.** Read at beamtime scope and INHERITED — `reconstruct` selects shared candidates by the reading's own `scope`, never by which file it came from, so a sample-specific remark inside a beamtime document cannot be attached to all 94 measurements. **A real defect the corpus found:** the beamsize section was SILENTLY SKIPPED because the label pattern allowed only letters and spaces and never matched a label containing `@` | `BL15R-003` |
| `BL15R-009` | Beamtime-note integration | **DONE 2026-09-16 for the `.txt`; DOCX and PDF extraction deliberately not built** and the module says the trio is one witness. **A FABRICATION DEFECT THE CORPUS FOUND, and the most important one in the programme:** the reader was INVENTING file numbers — pairing *any two* bare integers read one sample's STEP numbers as FILE numbers, producing `2, 4, 6, 8, 10` for a sample that has none, because an empty DOCX cell flattens to a lone tab that a blank-line skip swallowed. Fixed by requiring ADJACENCY, with an empty cell DISCLOSED rather than filled; measured after, file numbers read are exactly 11-51, 41 rows, contiguous. The broad *"In ALL experiments…"* claim is read at beamtime scope and **applied to nothing** | `BL15R-003` |
...
| `UX-020` (eyebrow containment inverted) | **OPEN, AND NOW ACTIONABLE RATHER THAN JUST OPEN — its premise is corrected and its blast radius measured, 2026-09-15. Deliberately NOT done in this branch; see why below.** | **The diagnosis is RIGHT**: the record eyebrow renders `workspaceLabel(activeView)` (`RecordWorkbench.tsx:963`), so `RECORD FIELDS` — a *narrower* sub-view — sits above the record title, while every other `.eyebrow` pair puts the broader category above. Confirmed in a rendered screenshot. **ONE CLAIM IN THE ROW IS WRONG AND THE CORRECTION HELPS IT**: `TopBar`'s `surface` prop is **not unused** — `ExportReadiness.tsx:565`, `EvidenceExplorer.tsx:214,281,347` and `GuidedCompletion.tsx:957` all pass it, for exactly this purpose. So the fix follows an ESTABLISHED sibling pattern (`RecordWorkbench` is the one record screen that omits it) rather than activating a dormant prop, and `stateChip={detail.exported ? 'exported' : 'draft'}` is already computed on the same call. **WHY IT IS ITS OWN SLICE:** `surfaces.ts:171` records that *"`Record Fields` is the `.eyebrow` inside that `h1`"*, and `:183` gates `record-detail` on `ready: { role: 'heading', name: /^Record Fields/ }` — so the eyebrow IS the record heading's accessible name. Changing it moves that ready gate for **every sweeping read-only spec across all five viewport projects**, plus `e2e/trusted/fixtures.ts`, two trusted specs and two bench files (6+ files, measured). That is a coordinated test-contract change, not a copy edit, and it is not a safe thing to land on a converging branch minutes from merge with only a degraded Impeccable run available. The next session should do it as one slice: eyebrow → state, workspace → `surface` crumb, and every ready gate updated in the same commit |
...
|---|---|---|---|
| **ACT-001** — **COMPLETE** | Design the append-only activity/event model: actor, action, object, field, before, after, timestamp, channel | nothing — **designable today** | Reuses two proven precedents: `db_write._APPEND_ONLY_TABLES` (mechanical no-`DELETE`) and `revision_history.py` (read module separate from write module). ~~**Needs migration `0006`**, which means it needs an approval packet and an operator act — so the design must be worth that cost before it is written~~ — **FALSE, AND OVERTURNED BY `DEC-49` ON 2026-09-17. Struck rather than deleted because this sentence is exactly the kind that silently converts buildable work into blocked work, and it had already done so once: it is the reason this row sat unbuilt.** No migration is needed. The `proposals` precedent is the answer and it was already in the tree: `proposals.STATE_KEY = "proposals"` is a top-level key in the **experiment state document**, and its own comment block records that *"no migration is required, because `Experiment.from_state` reads every optional key with `.get` and a default"* — so a document written before the feature existed hydrates to an empty history. `CLAUDE.md` §15's 2026-08-29 extension chose that **deliberately**, recording four occasions on which a table reached `db_write.OWNED_TABLES` before any committed sentence named it and stating that *"a fifth is avoidable by not needing one."* **`OWNED_TABLES` is therefore UNCHANGED by this work**, and two properties come free rather than by assertion: activity is durable exactly where the experiment document is durable, and an activity event is **structurally** invisible to `export.transform`, to every submission content signature, and to every run's `resolved_run_draft`, because it lives outside `draft` **COMPLETE 2026-09-18** (shipped PR #264; the model was live-verified this session — a five-act walk recorded every field the row specifies, and a REFUSED act recorded nothing, leaving `seq` dense with no burned position). |
| **ACT-002** — **COMPLETE** | Record the **channel** on every write path (`web`, `mcp`, `historical_import`, `system`) | `ACT-001` | The channel is knowable **today**, at every write site, with no external dependency. This is the half of `DEC-44` that is not blocked on `EXT-01` **COMPLETE 2026-09-18** (shipped PR #264). Live-verified: `web` on every HTTP act and `historical_import` on an archive attach, so the fourth channel value is reachable in practice and not only in the vocabulary. **`CHANNEL_SYSTEM` has no write site** — named residue. |
| **ACT-003** — **COMPLETE** | Activity History read API + scientist-facing UI | `ACT-001` | Must render `unattributed` honestly rather than hiding entries that lack an actor **COMPLETE** (PR #270) and **INDEPENDENTLY REVIEWED 2026-09-18** — the review it never got, which found three Important defects its own 30-test suite passed. Polish shipped as `ACT-003b`, **PR #272 / `b7120462` / `v0.0.255`**, a11y green on the exact head. Detail: `docs/session-closure-2026-09-18.md` §3. |
| **ACT-004** — **COMPLETE** | Statistics reads the activity history as a **summary**, never as its source of truth | `ACT-003` | `DEC-44` is explicit. This also interacts with `DEC-25`'s standing obligation that Statistics must earn its sidebar slot by becoming scientist-first. **BUILT 2026-09-18** on branch `feat/act-004-statistics`: `GET /api/activity/summary` (`apps/api/isaac_api/activity_summary.py`, documented operations 89 -> 90) and a `Workspace Activity` section on the Statistics **Overview** tab. **No migration, no table, `db_write.OWNED_TABLES` unchanged, no write path, no MCP operation** — the per-experiment activity read is not an MCP tool either, so the agent surface is unmoved. It is placed on Overview and deliberately **NOT** on My Stats: every figure is a workspace total, and `MyStats.tsx`'s first of six traps is a workspace total presented as personal. **Two figures were REFUSED rather than rendered** — a count of distinct collaborators (structurally `0` in this build, which is a claim about the people where the true statement is about the deployment; the wire carries event counts and an empty `attributed_actors` instead) and a historical-imports figure (the `Historical Imports` section one card away already answers that question from `GET /api/imports`, and a near-identically-named figure counting a different thing is a confusion defect). **The bound is disclosed and the measurement corrected the premise:** `EXPERIMENT_LIMIT = 500`, but the aggregation is ~32 ms against a ~275 ms hydration pass at that size, so the bound bounds the RESPONSE and the per-event loop and **not** the request — the module docstring carries the four-row table. **MERGED 2026-09-18 as PR #273 / `44760336` / `v0.0.257`**, after an independent review (two Important, three Minor, all fixed) and an a11y round that moved six `settings-explorer` cells. Browser-measured after merge: Statistics Overview **2763 px → 3242 px**, still under `DEC-25`'s 3,820 px bar. |
