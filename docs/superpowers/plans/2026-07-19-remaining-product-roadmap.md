...
1. **Ephemeral + shared hosted workspace** (biggest architectural reality): one auto-seeded demo experiment on `/tmp`, wiped on restart, one global key. Search (B) and the demo look thin over a single experiment, and no user work persists. *Not a blocker for "current project complete" (synthetic demo), but it caps demo richness.* **Decision (LOCKED, Q-CROSS-2):** keep ephemeral + shared + honest re-seed (no DB / no volume); add a **deterministic richer synthetic seed** (multiple synthetic experiments in varied states) via the existing filesystem seeding path — placed in **Phase 26**, verified in Stabilization, clearly labeled demo data, never fake product data.
   > **CORRECTION (2026-07-21, P27.0 — additive).** "Ephemeral · `/tmp` · wiped on restart · no volume" is **inaccurate**. A Railway volume (`isaac-metadata-assistant-volume` @ `/data/isaac-workspace`, `ISAAC_UI_WORKSPACE` points at it) was provisioned in Phase 20 and persists across restarts/redeploys (verified via `railway status`). The synthetic-only/shared/honest-reseed posture is unchanged; only the persistence characterization is corrected. See `2026-07-21-post-phase-26-master-execution-ledger.md` §4.
...
- Q-CROSS-2 → **LOCKED.** Hosted workspace stays **ephemeral + shared + synthetic + honest re-seed**; **no** production DB / Railway volume for demo polish. A **deterministic richer synthetic seed** is **required** (placed in Phase 26; §7 risk 1). *(Baseline correction: the workspace seeds **one** experiment, not eight.)* — **CORRECTION 2026-07-21 (P27.0):** "ephemeral · no volume" was inaccurate; a persistent Railway volume at `/data/isaac-workspace` exists and is used. See ledger §4.
- **POST-PHASE-26 EXTENSION → APPROVED 2026-07-21.** Phases **27–32** (Runtime Safety · Workflow & Evidence Contracts · Assistant Experience · Live Runtime Record Retrieval · Synthetic/Public File Ingestion · UI Refinement+Stabilization+Docs+Release) extend the terminal arc. Promotes into **synthetic-only** scope: live runtime record retrieval; sandboxed synthetic/public ingestion. Does **not** promote real-data/LLM/vector/institutional items (§11 registry otherwise unchanged). Canonical tracker: **`2026-07-21-post-phase-26-master-execution-ledger.md`** — that ledger governs post-P26 execution.
