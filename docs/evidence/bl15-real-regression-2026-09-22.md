...
check the new semantics against real data. **Re-run 2026-09-23 on `bl15.mapping.cardinality.v2`** after the final independent review (C1) — the table below is v2; the v1 figures are kept in the note beneath it. **Only structural counts and concept names are
...
| Candidates | **200**: 74 sources agree · 51 single source · **29 varies** · **46 sources conflict** |
| Field conflicts by concept | acquisition_timestamp 9 · sample_position 9 · emission_energy 8 · spec_user_string 7 · scan_command 3 · counting_time 2 · energy_grid 2 · filter 2 · legacy_run_or_file_number 2 · cycling_state 2 |
| "Varies" (per-scan-item / per-file, ≥ 2 established scans — not conflicts) | detector_column 9 · motor_position 9 · acquisition_target 7 · unknown_token 4 |
...
| Add to Experiment (new record, `create_runs: true`) | 90 Runs created · 90 proposals (all `timestamps.acquired_start_utc`, **all 90 explicit-UTC ISO values** — no zone-less local time proposed into a UTC field) · **0 nominal offers** · 0 temperature values written · 90 run origins recorded |
...
**v1 → v2.** Under v1 the four quantities `emission_energy`, `energy_grid`, `counting_time` and `scan_command` were treated as per-scan, which made 15 of their readings "varies" (44 varies / 31 conflicts). The final independent review showed v1 could also hide a planned-vs-recorded disagreement on a single scan; v2 always compares measurement-level readings with scan-level ones, requires ≥ 2 established scans for variation, and returns those four to per-measurement **pending the domain owner (Q21)** — hence 29 varies / 46 conflicts. Every other row is unchanged between v1 and v2.

**Open, for the domain owner:** whether the 9 `acquisition_timestamp` and 9 `sample_position`
conflicts — and now the 15 readings of the four quantities above (Q21) — are genuine source disagreements or legitimate scan-to-scan variation. The pipeline
