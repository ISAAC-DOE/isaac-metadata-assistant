...
**A second closure slice (2026-08-26) fixed A11Y-04 and A11Y-05** and deleted their baseline
entries the same way — 13 nodes off both platform totals.

~~"fixed A11Y-04, A11Y-05 **and A11Y-06** … 27 nodes off both platform totals"~~ —
**CORRECTED 2026-08-27 from CI run 33025558592, and struck rather than rewritten because the
correction is the point.** A11Y-06 is **HALF closed**: the two `role="search"` landmarks really
are named now, but the entry's OTHER node per pair was never a `role="search"` landmark, and CI
...
* **A11Y-01** — open, and still a palette decision the owner has to make, explicitly not a
  ...
  **UPDATED 2026-09-12, not a correction of the claim above but an addition after it: the
  owner's palette decision WAS made and partially shipped as A3 (2026-08-31) — `--text-tertiary`
  `#78838f` → `#626c77`, a real token-VALUE change, pinned by
  `apps/web/src/__tests__/palette-contrast.test.ts` and `apps/web/src/styles/tokens.css:220`.**
  **A11Y-01 IS STILL OPEN** — `apps/web/e2e/a11y-baseline.ts` says so explicitly at its own
  post-A3 measurement ("WHAT THE REMAINING 871 ARE, so nobody reads this as A11Y-01 closed") —
  because A3 addressed only the "neutral tokens simply too light" cause; the `opacity`-composite
  cause and the saturated `--verified-text`/`--src-derivation` cause are unchanged and account
  for the residue (measured 871 nodes at the time A3 landed; re-measured here 2026-09-12 as
  **877**, see `A11Y_BASELINE_TOTAL_NODES` in `e2e/a11y-baseline.ts` — the file's own notes say
  this total covers every axe rule, not only `color-contrast`, so it does not by itself say how
  many of the 877 are still A11Y-01). See §6.1a's addition below for the same correction.
...
`global-setup.ts` does four things, and fails naming the step that failed. It asserts the
backend is up; refuses to run unless it reports `mode: "synthetic-only"`; asserts the
**ordinary workspace is empty**; and then opens **one** worked-example session for the
whole run (`POST /api/tutorial/sessions`), verifying by measurement that the five example
records are in it and publishing its id to the workers. If the backend is missing you get
the exact command to run, not 500 browser timeouts.

> **Corrected.** This paragraph used to say the setup "seeds the synthetic workspace
> **once** through `GET /api/experiments` — whose `ensure_seeded()` materialises every
> missing canonical id". Reads no longer seed and `ensure_seeded()` no longer exists
> (`apps/api/isaac_api/workspace.py`: `list_experiments` / `load_experiment` are
> documented "NEVER seeds", and `_materialise_seed` requires a `session_id`). The five
> examples exist only inside a session, and the record-count assertion did not weaken
> when it moved there — it is now against the five ids the backend reports as
> materialised, not a bare `>= 5` on a list.
...
**ADDED 2026-09-12 — the owner's decision for cause 1 above was made two days later, and this
section predates it.** A3 (2026-08-31) darkened `--text-tertiary` `#78838f` → `#626c77` (a real
TOKEN VALUE change, unlike the two usage corrections this section describes) and retired
`--text-quaternary` as an alias of it; pinned by `apps/web/src/__tests__/palette-contrast.test.ts`
and visible at `apps/web/src/styles/tokens.css:220`. That closes cause 1 as listed above. Causes 2
(`--verified-text #2f7d78` / `--src-derivation #7a6bb0`) and 3 (`opacity` composites) are
untouched, and `apps/web/e2e/a11y-baseline.ts` says explicitly, at its own post-A3 measurement,
that A11Y-01 remains open for that reason (871 residual nodes there; **877** re-measured here
2026-09-12 in `A11Y_BASELINE_TOTAL_NODES`, a total over every axe rule rather than
`color-contrast` alone). See CLAUDE.md §11's 2026-08-31 session entry for the fuller account of A3.
...
* **No screen-reader testing.** axe checks the accessibility *tree*; it does not tell you what
  ...
  button" or the pressed state audibly. **A11Y-06's search half has since been fixed and inherits
  the same limit**: the two `role="search"` landmarks now carry distinct names in the tree, and
  nothing here has heard a screen reader read them out. Its other half is still open — two
  `region` landmarks both named "Endpoint Explorer", which is precisely the confusion a landmark
  list makes audible and an axe count only counts. The same caveat applies to A11Y-04's new tab
  stops — that an element is focusable and named is a tree fact, not a proof that arrowing
  through it reads sensibly.
