...
| 5 | **Demo idempotence** | On `/experiments` the **Run Synthetic Demo** button navigates to `/load`; the run itself is the "Run the Synthetic Demo" panel there (`POST /api/demo/run`, `apps/web/src/screens/LoadMaterials.tsx:33-47, 83`). Run it twice: the list stays at **five** records — the run targets fixed canonical ids and never appends. Verified: 5 → 5 → 5, target id `01SYNTHXANESSEED0000000001` both times. | If a sixth appears, do not run the demo during the meeting; use Reset Demo. **CORRECTED 2026-08-01 (W1):** this cell previously warned that *"a demo run rewrites scenario 1 to its baseline, so it will discard the confirmation you make in step 5 — do not run it mid-demo"*. That was true, and it was a **defect**, not a property. It is fixed: a run over an edited scenario is now **refused with 409** and writes nothing, and the UI says so and points at Reset Demo. On an untouched scenario the run writes nothing either — it is a true no-op. **Running it mid-demo is now safe**, and if you have confirmed something in step 5 you should expect the refusal rather than a silent reset. |
